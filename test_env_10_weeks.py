...
def test_time(tables, n, w, weight, df):
    start1 = time.time()
    chunks = []
...
def overnight_test():
    ...
    for index in range(1, 100):
        ...
        margin, changes, benchmark_changes, ttl_margins, chunks = test_time(tables, index, 1, True, df2)
        ...
        margins_obtained_ultra.append((index, series.median(), series.mean(), pct_positive, sortino))
       
    for item in margins_obtained:
        print(item)
    most_successful = max(margins_obtained, key=lambda item: item[1])

    pd.DataFrame(margins_obtained_ultra, columns=['n', 'median', 'mean', 'win_rate', 'sortino']).to_csv('results.csv', index=False)
...
def single_run():
    ...
    average_margin, changes, benchmark_changes, ttl_margins, chunks = test_time(tables, int(number), 1, True, df2)
